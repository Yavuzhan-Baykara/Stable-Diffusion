...
class UNet_Tranformer(nn.Module):
    def __init__(self, marginal_prob_std, channels=[32, 64, 128, 256], embed_dim=256, text_dim=256, nClass=10):
        ...
        self.conv1 = nn.Conv2d(3, channels[0], 3, stride=1, padding=1, bias=False)  # 3-channel input for CIFAR-10
        self.dense1 = Dense(embed_dim, channels[0])
        self.gnorm1 = nn.GroupNorm(4, num_channels=channels[0])
        ...
        self.dense7 = Dense(embed_dim, channels[0])
        self.tgnorm2 = nn.GroupNorm(32, num_channels=channels[0])
        self.tconv1 = nn.ConvTranspose2d(channels[0], 3, 3, stride=1, padding=1)  # 3-channel output for CIFAR-10
        self.act = nn.SiLU()
        self.marginal_prob_std = marginal_prob_std
